...
struct CastData2{
    ...
    std::vector<CastData*> castChain; // links to related casts
    // meta
    ...
    std::string containerFunction;   // Function where cast expression appears
    ...
    std::string fromParameterInfo;
    std::string toParameterInfo;
...
std::ostream& dumpcsvChain(std::ostream &os, std::vector<CastData*> const & list);
...
public:
    void run(MatchFinder::MatchResult const &result) override {
        ...
        CensusNode cast;
        if(!!var) {
            cast = buildCastData(*context, *castExpr, *castSource, var, location);
        }
        else if (!!call) {
            cast = buildCastData(*context, *castExpr, *castSource, call, location);
        }
