...
public:
    void run(MatchFinder::MatchResult const &result) override {
        ...
        */
        ...
        /*
        auto castLinker = [&](auto const& censusNode) -> auto {
            // auto const& keyDecl
        };
        */
        ...
        if(!!var) {
            ...
            std::find_if(fn->param_begin(), fn->param_end(),
                    [&] (auto const& parm) {
                        ...
                        auto castInfo = castSource->getNameInfo();
                        return parmName == castInfo.getName();
                    });

            if (parmPos > fn->getNumParams()) {
                src = src + " (Could not match with fn parameter)";
            }
            else {
                src = src + " " + toString(context, fn, parmPos-1);
            }
            */
